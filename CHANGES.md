...
# Single-tenant behavior synchronized into multi-tenant

Synchronization status: **applied to the multi-tenant architecture on 2026-08-30**. The 19 groups below remain as the functional specification and historical explanation of each fix. Paths inside those groups describe their single-tenant origin; the final section of this document records the actual multi-tenant destinations, deliberate architectural differences, generated contract and verification.
...
# Multi-tenant synchronization applied

The merge was resolved by behavior, not by selecting either side wholesale. Multi-tenant `HEAD` remained authoritative for structure, organization resolution and isolation; the incoming single-tenant branch remained authoritative for the user-visible fixes and features described above.

## Structural decisions and deliberately omitted legacy files

- `backend/routers/admin.py` remains deleted. Its incoming behavior was distributed among the tenant-aware routers in `backend/modules/restaurant/routers/`: `_shared.py`, `catalog.py`, `orders.py`, `customers.py`, `owner.py`, `reviews.py`, `checkout.py`, `products.py` and `profile.py`.
- No flat single-tenant `backend/models`, `backend/schemas`, `backend/services` or legacy admin router tree was recreated. Models and schemas remain under `backend/modules/auth` and `backend/modules/restaurant` and are exported through the existing multi-tenant compatibility surface only where that architecture already required it.
- No migration was added or copied. Existing revision identifiers, `organization_id` columns, PostgreSQL types and `VARCHAR` enum representation remain unchanged. The Docker smoke test exposed one pre-existing PostgreSQL portability defect in `20260825_0003_organization_experience.py`: the physical foreign-key name for `organization_feature_entitlement.organization_id` exceeded PostgreSQL's 63-character identifier limit. That name alone was shortened to `fk_org_feature_entitlement_organization`; table, columns, relationship, data operations, revision and downgrade behavior are unchanged.
- `frontend/src/features/ordering/routes/OrdersIndexRedirect.tsx` remains deleted. `/orders` is now the real guest-order index registered by `frontend/src/features/ordering/definition.ts` and rendered by `frontend/src/pages/GuestOrders.tsx`.
- The frontend still resolves `currentManifest`, capabilities and page sections before registering feature routes. The port did not replace manifest routing with the single-tenant static router.
- `publicApiClient`, `customerApiClient` and `adminApiClient` remain the request boundary and continue to add `X-Organization-Slug`. No service bypasses these clients.
- OpenAPI and `frontend/src/api/generated` were regenerated from the merged multi-tenant backend. No generated file was conflict-resolved or edited by hand.

## Backend mapping and final behavior

| Single-tenant responsibility | Multi-tenant implementation | Isolation retained |
|---|---|---|
| Admin order helpers and role transitions | `backend/modules/restaurant/routers/_shared.py`, `orders.py` | `require_organization_role(...)`, feature dependency and organization-scoped session |
| Product, ingredient and category directories | `backend/modules/restaurant/routers/catalog.py` | Tenant-scoped reads/mutations; structural CRUD restricted to Manager/Owner |
| Public menu, facets and reviews | `backend/modules/restaurant/routers/products.py`, `reviews.py` | Catalog/reviews entitlement and organization predicates |
| Checkout, guest order access and batch claim | `backend/modules/restaurant/routers/checkout.py` | Claim query explicitly requires the active `organization_id`; no cross-tenant transfer |
| Customer history and overview | `backend/modules/restaurant/routers/profile.py` | Authenticated customer plus active organization scope |
| Customer and staff directories | `backend/modules/restaurant/routers/customers.py`, `owner.py` | Owner-only organization role dependencies |
| Shared API envelopes and summaries | `backend/modules/restaurant/schemas/pagination.py`, `checkout.py`, `owner.py`, `product.py`, `review.py` | Public English field names and unchanged organization model |
| Registration consent | `backend/modules/auth/schemas/user.py`, `backend/modules/auth/services/authentication.py` | Existing organization-aware registration/authentication flow retained |
| Local network development entry point | `backend/app.py` | Existing app factory, organization middleware, feature routers and CORS retained; direct Uvicorn execution binds `0.0.0.0` |

The merged API now includes the intended contracts without introducing a database migration:

- `POST /checkout/orders/claim` performs partial, idempotent guest-order association. It selects only orders from the authenticated organization, refuses owned/expired/invalid/cross-tenant entries, clears a claimed guest token and does not award retroactive loyalty benefits.
- `DELETE /admin/orders/{order_id}` allows Manager/Owner to permanently delete only an already-cancelled order.
- `GET /admin/reviews`, `GET /admin/ingredients/{ingredient_id}/products` and `GET /profile/overview` replace full-collection and N+1 frontend work with tenant-scoped server queries.
- Growing collections return `items`, `page`, `per_page`, `total` and `total_pages`, with database filters/counts before `offset`/`limit`, stable ordering and the documented facets/summaries. Counter and Kitchen remain complete operational lists.
- Chef, Waiter, Manager and Owner can read the operational catalog and change availability through dedicated endpoints. Only Manager/Owner may create, structurally edit, archive, restore or delete catalog entities.
- Chef sees Counter read-only and operates Kitchen; Waiter operates Counter and Kitchen; Manager/Owner also use unrestricted Management. Only Manager/Owner may force arbitrary order states or delete cancelled orders.
- Checkout contact e-mail and phone are optional but validated when supplied. Registration requires `accepted_terms=true` without a new persisted field.
- Boolean product/customization predicates use `.is_(True/False)` so PostgreSQL no longer receives integer comparisons for Boolean columns. ORM writes for product flags and ingredient-link flags also pass native booleans instead of converting them to `1`/`0`; the only remaining `is_primary = 1` expressions are deliberately SQLite-only partial-index or offline SQLite snapshot SQL paired with PostgreSQL's native Boolean predicate.

Backend files added or changed for the synchronization:

- `backend/app.py`
- `backend/alembic/versions/20260825_0003_organization_experience.py` (constraint-name portability fix only; no new revision)
- `backend/modules/auth/schemas/user.py`
- `backend/modules/auth/services/authentication.py`
- `backend/modules/restaurant/routers/_shared.py`
- `backend/modules/restaurant/routers/cart.py`
- `backend/modules/restaurant/routers/catalog.py`
- `backend/modules/restaurant/routers/checkout.py`
- `backend/modules/restaurant/routers/customers.py`
- `backend/modules/restaurant/routers/orders.py`
- `backend/modules/restaurant/routers/owner.py`
- `backend/modules/restaurant/routers/products.py`
- `backend/modules/restaurant/routers/profile.py`
- `backend/modules/restaurant/routers/reviews.py`
- `backend/modules/restaurant/schemas/checkout.py`
- `backend/modules/restaurant/schemas/owner.py`
- `backend/modules/restaurant/schemas/pagination.py`
- `backend/modules/restaurant/schemas/product.py`
- `backend/modules/restaurant/schemas/review.py`
- `backend/tests/test_endpoint_smoke.py`
- `backend/tests/test_multitenancy.py`
- `backend/tests/test_rate_limit.py`
- `backend/tests/test_sqlalchemy2_behavior.py`
- `backend/tests/test_validation.py`

## Frontend mapping and tenant-aware behavior

- `frontend/src/App.tsx` keeps manifest/capability routing while adding global legal pages, the real ordering routes, the unified administrative console and compatible redirects from `/admin/super`, `/admin/staff` and `/admin/kitchen`.
- `frontend/src/pages/AdminDashboard.tsx` and `AdminDashboard.css` host the role-aware Counter/Kitchen/Management views, server pagination, translated filters, collapsed operational columns, product/ingredient/category/customer/staff directories, related-product modal and adaptive editors. The selected order view and paginated filter state remain in the URL.
- `frontend/src/utils/adminOrderViews.ts`, `adminEnumLabels.ts`, `orderProgress.ts`, `adaptivePanelMode.ts` and `productAnalyticsView.ts` centralize permissions, translated enums, order progress and valid panel preferences; their corresponding `*.test.ts` files protect the helpers from divergent UI checks.
- `frontend/src/components/admin/AdaptivePanel.tsx` is the shared animated drawer/modal surface for analytics and editors. Its stored preferences are organization-scoped, desktop drawer mode is non-blocking, modal mode blurs the complete console, and mobile keeps the administrative header visible.
- `frontend/src/components/ui/Pagination.tsx` and `CustomSelect.tsx` provide the themed paginator and animated select used by storefront and admin. Pagination changes URL state without scrolling the page to the top and reduces to a result count when only one page exists.
- `frontend/src/pages/GuestOrders.tsx`, `OrderDetails.tsx`, `components/OrderStatusBar.tsx` and `components/orderStatusStorage.ts` implement multiple simultaneous guest orders, per-order details/cancellation/receipt, consistent not-found presentation, progress animation and claim cleanup.
- `frontend/src/context/AuthContext.tsx` attempts batch claim after login, registration and authenticated hydration. Local tokens are removed only after a successful response; network failures keep them for the next attempt.
- `frontend/src/core/storage/organizationStorage.ts` scopes guest orders, authentication, cart, theme, admin panel modes and notice dismissal. Legacy Bonefree keys migrate only into the Bonefree organization; storage events compare the already-prefixed active key.
- `frontend/src/components/Navbar.tsx` keeps “Meus pedidos” visible for guests and authenticated customers, keeps the cart capability-aware and preserves the corrected overlay z-index. `Footer.tsx`, `Terms.tsx`, `Privacy.tsx`, `PrototypeNotice.tsx`, `Checkout.tsx` and `SiteThemeController.tsx` resolve names, contacts, notices, logo/favicon and title from the current organization with safe fallbacks.
- `frontend/src/services/*` continues to call the multi-tenant generated client wrappers, so pagination, claim, availability and media requests retain organization headers. The generated contract under `frontend/openapi` and `frontend/src/api/generated` is the merged multi-tenant contract.

Frontend implementation files added or materially changed:

- Routing/organization: `frontend/src/App.tsx`, `frontend/src/features/ordering/definition.ts`, `frontend/src/core/storage/organizationStorage.ts`, `frontend/src/context/AuthContext.tsx`
- Global shell: `frontend/src/components/Navbar.tsx`, `Footer.tsx`, `OrderStatusBar.tsx`, `PrototypeNotice.tsx`, `SiteThemeController.tsx`, their CSS files, `frontend/src/theme.css`, `frontend/index.html`, `frontend/public/favicon.svg`
- Storefront/account: `frontend/src/pages/Home.tsx`, `Menu.tsx`, `ProductDetail.tsx`, `Cart.css`, `Checkout.tsx`, `Checkout.css`, `Profile.tsx`, `Profile.css`, `GuestOrders.tsx`, `GuestOrders.css`, `OrderDetails.tsx`, `OrderDetails.css`, `Login.tsx`, `Register.tsx`, `Auth.css`, `Terms.tsx`, `Privacy.tsx`, `Legal.css`
- Admin: `frontend/src/pages/AdminDashboard.tsx`, `AdminDashboard.css`, `AdminLogin.tsx`, `frontend/src/components/admin-orders/*`, `frontend/src/components/admin/AdaptivePanel.tsx`, `AdaptivePanel.css`
- Shared controls/helpers: `frontend/src/components/ui/CustomSelect.tsx`, `CustomSelect.css`, `Pagination.tsx`, `Pagination.css`, `paginationRange.ts`, `frontend/src/utils/adminEnumLabels.ts`, `adminOrderViews.ts`, `orderProgress.ts`, `adaptivePanelMode.ts`, `productAnalyticsView.ts`
- API/services/types: `frontend/src/services/adminService.ts`, `authService.ts`, `cartService.ts`, `checkoutService.ts`, `guestOrderService.ts`, `productService.ts`, `frontend/src/types/admin.ts`, `checkout.ts`, `pagination.ts`, `product.ts`, `user.ts`
- Localization/configuration: `frontend/src/i18n/adminPhrases.ts`, `frontend/src/i18n/locales/pt-PT.ts`, `en-GB.ts`, `de-DE.ts`, `frontend/src/config/siteFlags.ts`
- Tests: `frontend/src/api/clients.test.ts`, `frontend/src/components/orderStatusStorage.test.ts`, `frontend/src/components/ui/Pagination.test.ts`, `frontend/src/components/admin-orders/orderUtils.test.ts`, `frontend/src/services/guestOrderService.test.ts` and all new helper `*.test.ts` files under `frontend/src/utils`

## Isolation and regression checks represented in tests

- `backend/tests/test_multitenancy.py` creates two organizations and verifies domain resolution, capability/profile isolation, guest-token denial and batch-claim rejection across organizations. The rejected cross-tenant order retains both its null owner and original token hash.
- Backend endpoint tests cover pagination metadata and limits, server-side filters/facets/summaries, optional checkout contacts, accepted terms, progressive staff permissions, unrestricted Management, cancelled-order deletion and PostgreSQL-compatible Boolean expressions.
- `frontend/src/components/orderStatusStorage.test.ts` switches the configured organization in one browser storage instance and verifies that each organization sees only its own guest-order collection.
- Frontend unit tests cover page-range ellipses, one-page behavior, URL-safe view fallbacks, enum labels, order progress, adaptive panel preference normalization, analytics presentation and guest claim preservation/removal semantics.

## Generated artifacts and verification commands

Contract artifacts are regenerated with `npm.cmd run api:generate` from `frontend` after backend changes:

- `frontend/openapi/openapi.json`
- `frontend/src/api/generated/index.ts`
- `frontend/src/api/generated/sdk.gen.ts`
- `frontend/src/api/generated/types.gen.ts`
- the generator-owned `frontend/src/api/generated/client` and `core` support files

The final validation sequence for this merge is:

1. `..\.venv\Scripts\python.exe -m unittest discover -s tests -v` from `backend`.
2. `npm.cmd test`, `npm.cmd run lint`, `npm.cmd run build` and, after staging generated changes, `npm.cmd run api:check` from `frontend`.
3. `git diff --check`, conflict-marker/import/storage audits and confirmation that no legacy flat backend module or migration was resurrected.
4. Docker/PostgreSQL smoke testing with two organizations for public product/CORS behavior, organization headers, guest claims and media/storage separation.

Validation completed on 2026-08-30:

- Backend: `169` tests passed and `1` PostgreSQL-baseline test was skipped because `TEST_POSTGRES_DATABASE_URL` was not supplied to the host unittest process. The same historical migration path was then exercised successfully by the real Docker PostgreSQL startup.
- Frontend: `21` test files and `72` tests passed; ESLint passed; the production build passed. Vite emitted only its existing non-fatal advisory that the administrative chunk is larger than 500 kB.
- Contract: `npm.cmd run api:check` exported `85` paths, regenerated the client and produced no unstaged difference from the indexed artifacts.
- Repository: no conflict marker, unmerged index entry, legacy flat backend directory/import or whitespace error remained. `OrdersIndexRedirect.tsx` remained deleted and no new Alembic revision was introduced.
