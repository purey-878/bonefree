...
from modules.restaurant.schemas import (
    CartOut,
    CartItemOut,
    AddItemSchema,
    UpdateItemSchema,
...
class CartCustomizationRow(TypedDict):
    ingredient_id: int | None
    option_id: int | None
    action: CartCustomizationAction
    quantity: int
    extra_price: Decimal
...
def _validate_and_build_customization(
...
) -> tuple[ItemCustomization, Decimal, list[CartCustomizationRow]]:
    ...
    customization_rows: list[CartCustomizationRow] = []
...
def trusted_guest_customization(
    ...
    quantity: int,
    customization: ItemCustomization | None,
) -> tuple[ItemCustomization | None, list[CartCustomizationRow]]:
    if not customization:
        return None, []
