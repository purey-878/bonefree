...
from modules.restaurant.models import CancellationOrigin, CouponType, EntityStatus, FulfillmentMethod, OrderState, PaymentMethod, PaymentState, PaymentStatus, normalize_enum
...
class OrderItemCreateData(TypedDict):
    ...
    discount_percentage_snapshot: Decimal
    unit_price: Decimal
    quantity: int
...
def _cart_items_for_user(db: Session, current_user: User) -> list[CheckoutItem]:
    cart = db.scalar(select(Cart).where(Cart.customer_id == current_user.id))
    if not cart:
        return []
...
def _clear_user_cart(db: Session, current_user: Optional[User]) -> None:
    if not current_user:
        return
...
def _get_valid_coupon(db: Session, current_user: User, code: str | None, subtotal: Decimal) -> tuple[Coupon | None, Decimal]:
...
def _get_or_create_loyalty(db: Session, current_user: User) -> CustomerLoyalty:
    loyalty = db.scalar(
        select(CustomerLoyalty).where(CustomerLoyalty.customer_id == current_user.id)
    )
    if not loyalty:
        loyalty = CustomerLoyalty(customer_id=current_user.id, orders_above_50=0, total_coupons_earned=0)
        db.add(loyalty)
        db.flush()
    return loyalty
...
def _new_coupon_code(db: Session, current_user: User, discount_type: str, discount_value: Decimal) -> str:
    prefix = _coupon_code_prefix(discount_type, discount_value)
    for _ in range(10):
        code = f"{prefix}-{current_user.id}-{uuid4().hex[:6].upper()}"
...
def _award_loyalty_coupon_if_eligible(db: Session, current_user: User, qualifying_subtotal: Decimal) -> str | None:
...
def create_order(
    ...
    subtotal = Decimal("0")
    order_items: list[OrderItemCreateData] = []
