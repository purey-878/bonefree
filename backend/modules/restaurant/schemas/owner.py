"""Legacy `/admin` schemas for restaurant owners and staff."""
...
class AdminResponse(BaseModel):
    ...
    admin_id: int = Field(validation_alias="id")
    name: str
    email: str
    role: UserRole
    status: UserStatus
...
class StaffAdminCreate(BaseModel):
    ...
    def validate_role(cls, value: str | UserRole) -> UserRole:
        ...
        if normalized not in ORGANIZATION_STAFF_ROLES:
...
class OrderResponse(BaseModel):
    ...
    # Public OpenAPI compatibility only; no legacy schemas package exists.
    __module__ = "schemas.admin"
