...
from modules.restaurant.models import ReviewReactionType, ReviewStatus
...
class ReviewReplyResponse(BaseModel):
    reply_id: int
    review_id: int
    admin_id: int = Field(validation_alias="author_user_id")
    text: str
    created_at: datetime
    updated_at: datetime
...
class ReviewReactionResponse(BaseModel):
    reaction_id: int
    review_id: int
    admin_id: int = Field(validation_alias="reacted_by_user_id")
    type: ReviewReactionType
    created_at: datetime
