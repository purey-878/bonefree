...
from modules.auth.models import OrganizationDomain, OrganizationProfile
from modules.restaurant.models import SiteSetting
from modules.auth.models import SocialLinksData
...
from modules.restaurant.models import SocialPlatform, ThemeId
from modules.auth.schemas.organization import OpeningHours
...
class ThemeColorsData(TypedDict):
    primary: str
    accent: str
    secondary: str
    background: str
...
class ThemeBackgroundData(TypedDict):
    type: Literal["solid", "gradient", "pattern"]
    value: str
    overlay: NotRequired[str]


class ThemeDecorationData(TypedDict):
    ...
    customSvg: NotRequired[str]
    count: NotRequired[int]
    color: NotRequired[str]


class ThemeFontsData(TypedDict, total=False):
    heading: str
    body: str


class ThemeUiData(TypedDict):
    borderRadius: str
    buttonStyle: Literal["rounded", "pill", "sharp"]
    cardShadow: str


class ThemeConfigData(TypedDict):
    id: str
    name: str
    colors: ThemeColorsData
    background: ThemeBackgroundData
    decorations: list[ThemeDecorationData]
    fonts: NotRequired[ThemeFontsData]
    ui: ThemeUiData
...
BUILT_IN_THEMES: dict[str, ThemeConfigData] = {
    "normal": {
        "id": "normal",
...
def get_site_theme_settings(db: Session) -> SiteThemeSettings:
    ...
    try:
        payload: object = json.loads(row.value)
