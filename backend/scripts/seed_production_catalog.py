...
def _synchronize_postgres_sequences(db: Session) -> None:
    ...
    for table_name in INSERT_ORDER:
        ...
        highest_id = int(
            db.scalar(
                select(func.max(model.id)).execution_options(
...
def main() -> None:
    ...
    parser.add_argument(
        "--organization-slug",
        default="bonefree",
        help="Organization that will own the catalog (default: bonefree).",
    )
