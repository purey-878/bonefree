...
const GUEST_ORDER_ACCESSES_KEY = 'guest_order_accesses_v1'
...
function notifyGuestOrdersUpdated() {
  if (typeof window !== 'undefined') window.dispatchEvent(new Event(GUEST_ORDERS_UPDATED_EVENT))
}
...
function parseStoredAccesses(): StoredGuestOrderAccesses {
  ...
  try {
    const parsed = JSON.parse(organizationStorage.getItem(GUEST_ORDER_ACCESSES_KEY) ?? '{}')
...
function writeStoredAccesses(accesses: StoredGuestOrderAccesses) {
  ...
  if (Object.keys(accesses).length === 0) {
    organizationStorage.removeItem(GUEST_ORDER_ACCESSES_KEY)
    return
  }
  organizationStorage.setItem(GUEST_ORDER_ACCESSES_KEY, JSON.stringify(accesses))
}

function migrateLegacyAccess(accesses: StoredGuestOrderAccesses) {
  if (!storageAvailable()) return accesses

  const orderId = Number(organizationStorage.getItem(LEGACY_ACTIVE_ORDER_KEY))
  const accessToken = organizationStorage.getItem(LEGACY_ACTIVE_ORDER_ACCESS_TOKEN_KEY)
  const accessExpiresAt = organizationStorage.getItem(LEGACY_ACTIVE_ORDER_ACCESS_EXPIRES_AT_KEY)
  ...
  organizationStorage.removeItem(LEGACY_ACTIVE_ORDER_KEY)
  organizationStorage.removeItem(LEGACY_ACTIVE_ORDER_ACCESS_TOKEN_KEY)
  organizationStorage.removeItem(LEGACY_ACTIVE_ORDER_ACCESS_EXPIRES_AT_KEY)
  return accesses
}
...
export function isGuestOrdersStorageEvent(event: StorageEvent): boolean {
  return event.key === organizationStorage.key(GUEST_ORDER_ACCESSES_KEY)
}
